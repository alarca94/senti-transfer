...
def read_datasets(datasets, tasks, lang='es'):
    ...
    for dataset in datasets:
        ...
        dataset_tasks = [t[0] if isinstance(t, tuple) else t for t in tasks[dataset]]
        for phase in data[dataset]:
            data[dataset][phase] = data[dataset][phase][['text'] + dataset_tasks]
...
def process_model_config(dic):
    for k, v in dic.items():
        if isinstance(v, dict):
            ...
            if isinstance(v[0], str) and v[0] in valid_functions:
                func_ = getattr(hp, v[0].replace(" ", "").split("(")[0])
                val_string = v[0].replace(" ", "").split("(")[1].split(")")[0] \
                    if len(v[0].replace(" ", "").split("(")) > 1 else None
                ...
                for vi in v:
                    if isinstance(vi, dict):
                        ...
                        dic[k] = dic[k][1:]
                dic[k] = hp.choice(k, dic[k])
            else:
                dic[k] = hp.choice(k, literal_eval("[" + str(",".join([str(v) for v in v])) + "]"))
        else:
            dic[k] = v
...
def process_config(dic, name=None):
    for k, v in dic.items():
        if k == 'models':
            process_model_config(dic[k])
    dic['retrain'] = dic.get('retrain', False)
    return dic
...
def print_gpu_usage(curr_phase=''):
    print(f'GPU allocation @ {curr_phase}: {torch.cuda.memory_allocated() / 1e9:.2f}GB')
    print(f'GPU cached @ {curr_phase}: {torch.cuda.memory_cached() / 1e9:.2f}GB')


def print_results(results, phase='train'):
    print(colored(f'Results corresponding to the {phase} phase:', Colors.BLUE))
    for k, v in results.items():
        if k.lower() == 'time':
            if phase.lower() == 'train':
                print(colored(f'\tMean Time Complexity (per epoch):', Colors.BLUE) + f' {v:.2f} seconds')
            else:
                print(colored(f'\tTime Complexity:', Colors.BLUE) + f' {v:.2f} seconds')
        else:
            print(colored(f'\t{k}:', Colors.BLUE) + f' {v:.4f}')


def print_model_config(model_name, config):
    print(colored(f'Loading {model_name} with the following configuration:', Colors.CYAN))
    for k, v in config.items():
        if k not in ['evaluator', 'n_classes', 'loss_fn', 'device', 'task_name', 'epochs']:
            print(colored(f'\t{k}:', Colors.CYAN) + f' {v}')
